Basic Touchpanel with the following functionality:
-Source Selection
-Microphone volume adjustment
-Source volume adjustment
-Blank screen
-End/Start class screens & buttons

Equipment:
-Crestron DMPS3-4K-350-C (DM Switcher)
-Crestron DM-RMC-4K-SCALER-C (Scaler)
-Crestron CEN-SW-POE-5 (PoE Switch)
-Crestron TSW-760-B-S (Touch Panel)
-Shure ULXD4 H50 Receiver W/SM58 Handheld Mic (Receiver/Mic Combo)
-Shure BLX4R H9 Receiver W/Lapel (Receiver/Mic Combo)
-Sharp LC-48lE55U (TV)

Known Issues:
-Active source needs to be selected for microphone to work
-Blank screen mutes microphones too

Changes From V1.0:
-Removed redundant blank screen variable
-WIP

Bugs Fixed:
-display_audio = 5d when no sources are present, should be 0d (V1.0)